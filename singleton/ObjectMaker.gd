...
@export var medusa_shoot_reference = preload("res://scenes/enemies/medusa/medusa_projectile.tscn")
...
func create_medusa_attack(spawn_position: Vector2, player_position: Vector2):
	print("creando projectil")
	var projectile = medusa_shoot_reference.instantiate()
	projectile.global_position = spawn_position
	var to_player = (player_position - spawn_position).normalized()
	projectile.set_direction(to_player)
	projectile.rotation = to_player.angle()
